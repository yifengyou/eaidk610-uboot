...
void rockchip_show_logo(void) {
    ...
    list_for_each_entry(s, &rockchip_display_list, head)
    {
        ...
        // load_bmp_logo 加载ulogo_name名称的logo bmp到s->logo(struct logo_info)
...
int rockchip_display_init(void) {
    ...
    fdt_for_each_subnode(blob, child, route)
    {
        ...
        printf("[YYF] logo,uboot:%s (setup ulogo_name)\n", s->ulogo_name);
        fdt_get_string(blob, child, "logo,kernel", &s->klogo_name);
        printf("[YYF] logo,kernel:%s (setup klogo_name)\n", s->klogo_name);
        fdt_get_string(blob, child, "logo,mode", &name);
        printf("[YYF] logo,mode:%s (setup mode)\n", name);
        ...
        printf("[YYF] charge_logo,mode:%s (setup charge_logo mode)\n", name);
        if (!strcmp(name, "fullscreen"))
