...
echo    "******************************"
make ARCH=arm  O=build CROSS_COMPILE="aarch64-linux-gnu-" rk3399_linux_defconfig
echo " * make rk3399_linux_defconfig done! [$?]"

echo    "******************************"
echo    "*     Make AArch64 Uboot     *"
echo    "******************************"
make ARCH=arm V=1 O=build CROSS_COMPILE="aarch64-linux-gnu-" ARCHV=aarch64 --jobs=`nproc`
echo " * make done! [$?]"
