...
static int get_disk_partition_index(const char *name)
{
...
static int add_linux_cmdline(PBootInfo info, const char *partname)
{
	...
	index = get_disk_partition_index(partname);
	if (index < 0) {
		printf("** Can't Get %s Parition Index.\n", partname);
		return -1;
	}

	printf("## Use Linux CmdLine.\n");
	sprintf(cmdline, "%s  root=/dev/mmcblk1p%d rootwait  rw rootfstype=ext4", info->cmd_line, index + 1);

	len = strlen("init=/init");
	p = strstr(cmdline, "init=/init");
	for (i = 0; i < len; i++) {
		p[i] = ' ';
	}

	strcpy(info->cmd_line, cmdline);
	//printf("After bootlinux Cmdline: %s\n", info->cmd_line);
...
static rk_boot_img_hdr * rk_load_image_from_storage(const disk_partition_t* ptn, bootm_headers_t *pimage)
{
	...
	if (memcmp(hdr->magic, BOOT_MAGIC,
		...
		if (gLinuxBoot == 0) {
			...
			add_linux_cmdline(&gBootInfo, PARTITION_LINUX);

			if (rkimage_load_image(hdr, ptn, get_disk_partition(KERNEL_NAME_L)) != 0) {
				FBTERR("bootrk: bad boot or kernel-L image\n");
				goto fail;
			}
		}
#endif
	...
	if (!content.load_addr) {
		...
		if (gLinuxBoot == 0) {
			...
			content = rkimage_load_fdt(get_disk_partition(RESOURCE_NAME_L));
		}
#endif
		#endif
	}
